  ...
  if (loading) {
    return (
      <div className="w-full bg-[#1E1E1E] rounded-3xl p-8 border border-[#444746] flex flex-col items-center justify-center min-h-[300px] gap-4">
        ...
        <p className="text-[#8AB4F8] font-medium animate-pulse">신원 통합 중...</p>
      </div>
    );
  }
    ...
    return (
      <div className="w-full bg-[#1E1E1E] rounded-3xl p-8 border border-[#444746] border-dashed flex items-center justify-center min-h-[300px]">
        <div className="text-center">
            ...
            <span className="text-[#757775]">트랜잭션 대기 중...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full bg-[#1E1E1E] rounded-3xl overflow-hidden border border-[#444746] shadow-xl transition-all duration-500">
      ...
      <div className="bg-[#004A77] px-6 py-3 flex justify-between items-center border-b border-[#00639B]">
        <div className="flex items-center gap-2">
            ...
            <span className="text-[#D3E3FD] font-medium tracking-wide text-sm">최종 고객 기록 생성됨 (Golden Record)</span>
        </div>
        <span className="text-[#78D9EC] font-mono text-xs opacity-80">신뢰도: {(data.confidence_score * 100).toFixed(0)}%</span>
      </div>
      ...
      <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
            <div>
                <label className="text-[11px] uppercase tracking-widest text-[#C4C7C5] mb-1 block">고객 성함</label>
            ...
            <div>
                <label className="text-[11px] uppercase tracking-widest text-[#C4C7C5] mb-1 block">이메일 주소</label>
                <div className={`p-3 rounded-lg bg-[#121212] font-mono text-sm border flex flex-wrap items-center gap-2 ${isUpdated('email') ? 'border-[#a8dab5] text-[#a8dab5] bg-[#a8dab5]/5' : 'border-transparent text-[#E3E3E3]'}`}>
                    ...
                    {isUpdated('email') && <span className="text-[10px] bg-[#a8dab5] text-[#003816] px-1.5 py-0.5 rounded uppercase font-bold shrink-0">업데이트됨</span>}
                </div>
            </div>

            <div>
                <label className="text-[11px] uppercase tracking-widest text-[#C4C7C5] mb-1 block">전화번호</label>
                <div className={`p-3 rounded-lg bg-[#121212] font-mono text-sm border flex flex-wrap items-center gap-2 ${isUpdated('phone') ? 'border-[#a8dab5] text-[#a8dab5] bg-[#a8dab5]/5' : 'border-transparent text-[#E3E3E3]'}`}>
                    <span>{data.phone || '기록 없음'}</span>
                    {isUpdated('phone') && <span className="text-[10px] bg-[#a8dab5] text-[#003816] px-1.5 py-0.5 rounded uppercase font-bold shrink-0">새로 추가됨</span>}
                </div>
            </div>
        </div>
        ...
        <div className="space-y-6">
             <div className="flex justify-between items-start">
                 <div>
                    <label className="text-[11px] uppercase tracking-widest text-[#C4C7C5] mb-1 block">현재 등급</label>
                    <div className="flex items-center gap-2">
             ...
             {data.updates_applied.length > 0 ? (
                 <div className="bg-[#1b5e20]/20 p-3 rounded-xl border border-[#1b5e20] flex gap-3 items-center">
                     ...
                     <div>
                         <span className="text-[#a8dab5] text-xs font-bold block">시스템 조치</span>
                         <span className="text-[#c4eed0] text-xs">상담 내용을 바탕으로 {data.updates_applied.join(', ')} 항목을 수정했습니다.</span>
                     </div>
                 </div>
             ) : (
                 <div className="bg-[#444746]/20 p-3 rounded-xl border border-[#444746] flex gap-3 items-center opacity-70">
                     ...
                     <div>
                         <span className="text-[#E3E3E3] text-xs font-bold block">변경 사항 없음</span>
                         <span className="text-[#C4C7C5] text-xs">기존 기록이 상담 내용과 일치합니다.</span>
                     </div>
                 </div>
             )}
        </div>
